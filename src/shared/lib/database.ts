...
class ChatDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(2).stores({
      sessions: '++id, session_id, provider_address, wallet_address, created_at, updated_at',
      messages: '++id, session_id, role, timestamp, provider_address',
      files: '++id, root_hash, wallet_address, network, uploaded_at'
    });
  }
}
...
class DatabaseManager {
  ...
  async saveFile(file: Omit<StoredFile, 'id'>): Promise<number> {
    const db = await this.getDB();
    return db.files.add(file) as Promise<number>;
  }

  async getFiles(walletAddress: string, network?: 'testnet' | 'mainnet'): Promise<StoredFile[]> {
    const db = await this.getDB();
    let query = db.files.where('wallet_address').equals(walletAddress);
    if (network) {
      query = query.and(f => f.network === network);
    }
    return query.reverse().sortBy('uploaded_at');
  }

  async getFileByRootHash(rootHash: string): Promise<StoredFile | undefined> {
    const db = await this.getDB();
    return db.files.where('root_hash').equals(rootHash).first();
  }
